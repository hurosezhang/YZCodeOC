...
//  是否是竖屏
#define IS_LANDSCAPE2 UIInterfaceOrientationIsLandscape([[UIApplication sharedApplication] statusBarOrientation])
#define IS_LANDSCAPE [YZScreen isLandscape]

// 屏幕宽高
#define SCREEN_WIDTH (IS_LANDSCAPE ? [[UIScreen mainScreen ] bounds].size.height : [[UIScreen mainScreen ] bounds].size.width)
#define SCREEN_HEIGHT (IS_LANDSCAPE ? [[UIScreen mainScreen ] bounds].size.width : [[UIScreen mainScreen ] bounds].size.height)

#define SCREEN_WIDTH2 (IS_LANDSCAPE2 ? [[UIScreen mainScreen ] bounds].size.height : [[UIScreen mainScreen ] bounds].size.width)
...
// 状态栏被隐藏时,高度为0
# define IS_IPHONEXMore (([YZScreen statusBarHeight] > 20)? YES : NO)
...
static inline NSInteger UIAdapter (float x){
    ...
    //  这个地方用的SCREEN_WIDTH2 适配横竖屏2种情况
    // 为什么不用SCREEN_WIDTH  应为；此处引用不了YZScreen 和其类方法
    CGFloat scale = 375 / SCREEN_WIDTH2;
